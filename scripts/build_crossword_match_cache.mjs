...
const OUTPUT_FILE = path.join(
  ...
  "games",
  "knowledge",
  "crosswordMatchCache.generated.js"
);
...
const buildLocaleContext = (locale) => {
  const source = Array.isArray(CROSSWORD_REPO_STYLE_BANK?.[locale])
    ? CROSSWORD_REPO_STYLE_BANK[locale]
    : [];
...
const generateCacheFile = async () => {
  const localeContexts = {
    es: buildLocaleContext("es"),
    en: buildLocaleContext("en")
  };
  ...
  for (let localeIndex = 0; localeIndex < LOCALES.length; localeIndex += 1) {
    ...
    for (let matchId = 0; matchId < MATCH_COUNT; matchId += 1) {
      ...
      const suffix = matchId === MATCH_COUNT - 1 ? "" : ",";
    ...
    await writeChunk(output, `  ]${localeSuffix}\n`);
  }
