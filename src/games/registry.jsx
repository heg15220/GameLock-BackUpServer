...
const TrayectoriaGame = lazy(() => import("./sports/trayectoria"));
...
export const CONTROL_HINTS_BY_LOCALE = {
  es: {
    ...
    "arcade-el-escondite":           "Tres rivales se esconden en siete sitios del parque. Clic/toque en un escondite o teclas 1-7 para mirar dentro. No hay temblores ni marcas que delaten dónde están. 5 búsquedas y 30 s. P pausa, R reinicia, M sonido y F pantalla completa.",
    ...
    "sports-trayectoria":          "Simulador de carrera futbolística: todo se juega con ratón o dedo, sin teclado. Rellenas la ficha (apellido, dorsal, pie, posición, selección y ritmo) y te sientas a negociar con uno de los tres clubes que te quieren a los 16: dos peticiones —más ficha, ser titular, bajar la cláusula, menos años— cada una con su probabilidad impresa, y luego firmas. El contrato ata de verdad: mientras corra, el club no puede echarte, y la ficha se cotiza como un rol, así que cobrar de titular y jugar de suplente te lo cobra la grada cada temporada. Cada ciclo respondes a una carta de decisión —las probabilidades impresas en la carta son las que se tiran— y después juegas los tres partidos que deciden la temporada: final continental, Mundial, ascenso, permanencia, semifinal o clásico. En cada uno eliges dónde colocas el disparo entre tres opciones; el hueco se sorteó antes de que eligieras y se revela después, y si tu OVR es alto una opción sale tachada porque el portero ya la cubre. Marcar o fallar decide ahí mismo el título, el ascenso o el descenso. Luego lees la portada de la temporada: partidos, goles, rol, títulos y premios. En el mercado de verano eliges club mirando el medidor de distancia: tu OVR menos el nivel de la plantilla es lo que decide minutos, goles y convocatorias, así que elegir club es elegir ese número. Enfrente tira la idolatría: crece quedándote en un club y se paga al irte, con el coste impreso en cada oferta antes de firmar. A los 40 te retiras y la vitrina se compara con la de una sombra que nació el mismo año.",
    ...
    "arcade-brile":                "Balón prisionero 6v6 en un cementerio vallado. Manejas a un jugador del equipo azul: WASD/flechas mueven (o arrastra / cruceta en móvil), Espacio/J lanza al rival más peligroso y K/Mayús atrapa (púlsalo al vuelo: te salvas y te quedas el balón). Da a un rival antes de que bote y va al cementerio; desde ahí sigue lanzando y vuelve si acierta. Gana quien vacía el campo rival. Enter empieza, P pausa, R reinicia, M sonido y F pantalla completa.",
  ...
  en: {
    ...
    "arcade-el-escondite":           "Three rivals hide across seven places in the park. Click or tap a hiding place, or press 1-7, to look inside. No shake or marker reveals where they are. 5 searches and 30s. P pauses, R restarts, M sound, F fullscreen.",
    ...
    "sports-trayectoria":          "A football career simulator played entirely with the mouse or a finger, no keyboard. Fill in the registration card (surname, number, foot, position, nation and pace) and sit down to negotiate with one of the three clubs that want you at 16: two asks — more wage, a starting place, a lower buy-out, fewer years — each with its odds printed, and then you sign. The contract genuinely binds: while it runs the club cannot push you out, and the wage is quoted as a role, so signing as a starter and playing as a substitute is charged to you by the crowd every season. Each cycle you answer a decision card — the odds printed on it are the odds that get rolled — and then play the three matches that decide the season: a continental final, a World Cup final, a promotion play-off, a survival decider, a semi-final or the derby. In each one you choose one of three placements; the gap was drawn before you chose and is revealed after, and a high OVR strikes one option out because the keeper has it covered. Scoring or missing settles the title, the promotion or the drop right there. Then you read the season's front page: matches, goals, role, trophies and awards. In the summer market you pick a club by reading the distance meter: your OVR minus the level of the squad around you is what decides minutes, goals and call-ups, so choosing a club is choosing that number. Pulling the other way is idolatry: it grows by staying at a club and is paid when you leave, with the cost printed on every offer before you sign. At 40 you retire and your cabinet is set against a shadow born the same year as you.",
    ...
    "arcade-brile":                "6v6 dodgeball in a fenced graveyard. You control one blue-team player: WASD/arrows move (or drag / D-pad on mobile), Space/J throws at the most dangerous rival and K/Shift catches (snatch it midair: you're safe and you keep the ball). Hit a rival before it bounces and they go to the graveyard; from there they keep throwing and return on a hit. Empty the enemy court to win. Enter starts, P pauses, R restarts, M sound, and F toggles fullscreen.",
