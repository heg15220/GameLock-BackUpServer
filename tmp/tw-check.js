// src/games/arcade/territory-war/index.jsx
...
var THROW_BASE = 200;
var THROW_SCALE = 400;
var BLAST_RADIUS = 108;
var BLAST_DAMAGE = 90;
var MAX_JUMP_UP = 76;
var MAX_JUMP_GAP_UP = 210;
var MAX_JUMP_GAP_FLAT = 250;
var MAX_JUMP_GAP_DOWN = 340;
var AIM_MIN = 0;
var AIM_MAX = 359;
var CANNON_USE_RANGE = 36;
var CANNON_SPEED_BONUS = 170;
var CANNON_WIND_FACTOR = 0.22;
var CANNON_MIN_TARGET_DISTANCE = 240;
var AI_RECENT_HISTORY = 14;
var TERRAIN_PROFILE = {
...
var MAPS = [
  {
    ...
    name: "Wasteland",
    water: false,
    skyColors: ["#5fa8e8", "#b8d8ff"],
    mountainColor: "#7a9ab8",
    groundColor: "#2d7a3a",
    dirtColor: "#6b4226",
    platforms: [
  ...
  },
  {
    ...
    name: "Skyline Ramps",
    water: false,
    skyColors: ["#8ebce6", "#e8f4ff"],
    mountainColor: "#8aa2ba",
    groundColor: "#3b7f54",
    dirtColor: "#5d3f28",
    platforms: [
  ...
  },
  {
    ...
    name: "Sunken Harbor",
    water: true,
    skyColors: ["#4f9ccf", "#c6e8ff"],
    mountainColor: "#6a90ad",
    groundColor: "#1f8a5b",
    dirtColor: "#584226",
    platforms: [
  ...
  },
  {
    ...
    name: "Iron Forge",
    water: false,
    skyColors: ["#d9a26a", "#ffe0b3"],
    mountainColor: "#a06f45",
    groundColor: "#855d16",
    dirtColor: "#6a4713",
    platforms: [
      { x: -40, y: 420, w: 280, h: 160, type: "ground" },
  ...
  },
  {
    ...
    name: "Overgrowth",
    water: false,
    skyColors: ["#86d49d", "#e5ffe8"],
    mountainColor: "#6da37c",
    groundColor: "#2d7c3c",
    dirtColor: "#4f3c22",
    platforms: [
  ...
  },
  {
    ...
    name: "Icebridge",
    water: true,
    skyColors: ["#97d4ff", "#f1fbff"],
    mountainColor: "#94aac5",
    groundColor: "#3e826a",
    dirtColor: "#4d4b4a",
    platforms: [
  ...
  },
  {
    ...
    name: "Catacombs",
    water: false,
    skyColors: ["#6f7786", "#c5ccd7"],
    mountainColor: "#7a8092",
    groundColor: "#4b5563",
    dirtColor: "#3b3029",
    platforms: [
  ...
  },
  {
    ...
    name: "Frontier Ridge",
    water: false,
    skyColors: ["#7db7e5", "#dbefff"],
    mountainColor: "#8799ac",
    groundColor: "#4e7d34",
    dirtColor: "#6b4c2e",
    platforms: [
  ...
  },
  {
    ...
    name: "Floodgate",
    water: true,
    skyColors: ["#5ca6cc", "#ccecff"],
    mountainColor: "#6c8ea6",
    groundColor: "#2f7f50",
    dirtColor: "#57432b",
    platforms: [
  ...
  },
  {
    ...
    name: "Sky Ruins",
    water: false,
    skyColors: ["#9ec3ef", "#ecf6ff"],
    mountainColor: "#91a3b8",
    groundColor: "#587d55",
    dirtColor: "#5d4932",
    platforms: [
  ...
  },
  {
    ...
    name: "Quarry",
    water: false,
    skyColors: ["#bfc9d9", "#eef2f8"],
    mountainColor: "#9ca6b6",
    groundColor: "#6e7c3d",
    dirtColor: "#5a4731",
    platforms: [
  ...
  },
  {
    ...
    name: "Citadel",
    water: false,
    skyColors: ["#9aa6bd", "#e2e8f5"],
    mountainColor: "#8b94aa",
    groundColor: "#49653d",
    dirtColor: "#4a3829",
    platforms: [
  ...
  },
  {
    ...
    name: "Eclipse Basin",
    water: true,
    skyColors: ["#6366a8", "#c7cbff"],
    mountainColor: "#7478a8",
    groundColor: "#355f44",
    dirtColor: "#45322a",
    platforms: [
...
function createInput() {
  const down = /* @__PURE__ */ new Set(), pressed = /* @__PURE__ */ new Set();
  return {
    press(c) {
      if (!down.has(c))
        pressed.add(c);
...
function buildMapCannons(map) {
  ...
  return [
    {
      id: `${map.id}-red-cannon`,
      team: "red",
      x: clamp(redAnchor.x + 26, 30, WIDTH - 30),
      y: redAnchor.y,
      dir: 1,
      powerBoost: CANNON_SPEED_BONUS,
      aimMin: AIM_MIN,
      aimMax: AIM_MAX,
      rangeMin: CANNON_MIN_TARGET_DISTANCE
    },
    {
      id: `${map.id}-blue-cannon`,
      team: "blue",
      x: clamp(blueAnchor.x - 26, 30, WIDTH - 30),
      y: blueAnchor.y,
      dir: -1,
      powerBoost: CANNON_SPEED_BONUS,
      aimMin: AIM_MIN,
...
function defaultAim(team) {
  return team === "red" ? 330 : 210;
}
function createState({ mapId = "wasteland", teamSize = 3, mode = "menu" }) {
  ...
  return {
    ...
    map,
    cannons,
    teamSize: clamp(teamSize, 1, 6),
    units,
    rng: createRng(),
    turn: {
      number: 1,
      team: "red",
      unitId: units.find((u) => u.team === "red")?.id ?? null,
      remainingMs: TURN_MS,
...
function startTurn(state, teamId) {
  ...
  if (TEAM[teamId].cpu) {
    const enemyTeamId2 = teamId === "red" ? "blue" : "red";
    const allyAlive = getAlive(state, teamId).length;
    const enemyAlive = getAlive(state, enemyTeamId2).length;
    ...
    if (Number.isFinite(prevX)) {
      ...
      if (movedSinceLastTurn < 34)
        state.ai.stagnantTurns = clamp(state.ai.stagnantTurns + 1, 0, 9);
      else
        state.ai.stagnantTurns = Math.max(0, state.ai.stagnantTurns - 1);
    }
...
var TOPOLOGY_CACHE = /* @__PURE__ */ new Map();
function platformBounds(p) {
  return { left: p.x + 16, right: p.x + p.w - 16, y: p.y };
}
function platformCenter(p) {
  return p.x + p.w * 0.5;
}
function intervalGap(al, ar, bl, br) {
  if (ar < bl)
    return bl - ar;
  if (br < al)
    return al - br;
...
function bestJumpLink(fp, tp) {
  ...
  if (a.right < b.left) {
    ...
    const ol = Math.max(a.left, b.left), or2 = Math.min(a.right, b.right);
    fromX = (ol + or2) * 0.5;
    toX = fromX;
  }
  const gap = intervalGap(a.left, a.right, b.left, b.right);
  const stepUp = a.y - b.y;
  if (stepUp > MAX_JUMP_UP)
    return null;
...
function topologyFor(map) {
  ...
  if (cached)
    return cached;
  const t = buildPlatformTopology(map);
  TOPOLOGY_CACHE.set(map.id, t);
  return t;
}
function reachablePlatformIndices(topology, startIndex) {
  if (startIndex < 0 || startIndex >= topology.nodes.length)
    return /* @__PURE__ */ new Set();
  const seen = /* @__PURE__ */ new Set([startIndex]), queue = [startIndex];
  while (queue.length > 0) {
    const node = queue.shift();
    for (const e of topology.edges[node]) {
      if (seen.has(e.to))
        continue;
      seen.add(e.to);
      queue.push(e.to);
...
function assertMapConnectivity(map) {
  ...
  const redZone = /* @__PURE__ */ new Set(), blueZone = /* @__PURE__ */ new Set();
  for (const sp of map.red) {
    const idx = findPlatformForPosition(map, sp.x, sp.y);
    if (idx >= 0)
      redZone.add(idx);
  }
  for (const sp of map.blue) {
    const idx = findPlatformForPosition(map, sp.x, sp.y);
    if (idx >= 0)
      blueZone.add(idx);
  }
  if (redZone.size === 0 || blueZone.size === 0)
    throw new Error(`Map "${map.id}" invalid spawn topology.`);
  const disconnected = [];
  ...
  if (disconnected.length > 0)
    throw new Error(`Map "${map.id}" disconnected zones (${disconnected.join(",")}).`);
}
...
function applyUnitPhysics(state, dt) {
  for (const unit of state.units) {
    if (!unit.alive)
      continue;
    if (unit.flashMs > 0)
      unit.flashMs = Math.max(0, unit.flashMs - dt * 1e3);
    unit.vy += GRAVITY * dt;
    ...
    for (const plat of state.map.platforms) {
      if (unit.x < plat.x + 5 || unit.x > plat.x + plat.w - 5)
        continue;
...
function chooseSimpleShotFromOrigin(originX, originY, target) {
  ...
  if (!best)
    return { angle: dx >= 0 ? 330 : 210, charge: clamp(0.44 + Math.abs(dx) / WIDTH * 0.34, 0.24, 0.95) };
  return { angle: best.angle, charge: best.charge };
}
function terrainProfileForPlatform(map, platformIndex) {
  if (platformIndex < 0 || platformIndex >= map.platforms.length)
    return TERRAIN_PROFILE.ground;
...
function pickPriorityEnemies(state, unit, enemies) {
  ...
  }).sort((a, b) => b.score - a.score).map((entry) => entry.enemy);
}
function evaluateCandidatePosition(state, candidate, unit, enemies, allies, aggression) {
  ...
  const terrainScore = lowHealth ? terrain.cover * 0.64 + terrain.stability * 0.36 : terrain.mobility * 0.52 + terrain.cover * 0.24 + terrain.stability * 0.24;
  ...
  for (const ally of allies) {
    if (!ally.alive || ally.id === unit.id)
      continue;
    const d = Math.hypot(ally.x - candidate.x, ally.y - candidate.y);
    if (d < 132)
      allyClusterPenalty += (132 - d) / 132;
  }
  let threatScore = 0;
  ...
  return edgeSafety * 34 + rangeScore * 36 + heightAdv * 27 + terrainScore * 32 + mobilityScore * 16 - threatScore * 42 - allyClusterPenalty * 24 - candidate.cost * 0.23 - waterPenalty;
}
function simulateProjectileLanding(state, ox, oy, angleDeg, charge, opts = {}) {
  ...
  for (let step = 0; step < 220; step++) {
    ...
    for (const plat of state.map.platforms) {
      ...
      if (p.x < l - p.radius || p.x > r + p.radius || p.y < top - p.radius || p.y > bot + p.radius)
        continue;
      if (py + p.radius <= top && p.y + p.radius >= top && p.vy > 0) {
        p.y = top - p.radius;
      ...
      if (py - p.radius >= bot && p.y - p.radius <= bot && p.vy < 0) {
        p.y = bot + p.radius;
        p.vy = Math.abs(p.vy) * 0.5;
        p.bounces++;
        continue;
      }
      if (px + p.radius <= l && p.x + p.radius >= l && p.vx > 0) {
        p.x = l - p.radius;
        p.vx = -Math.abs(p.vx) * 0.55;
        p.bounces++;
        continue;
      }
      if (px - p.radius >= r && p.x - p.radius <= r && p.vx < 0) {
        p.x = r + p.radius;
        p.vx = Math.abs(p.vx) * 0.55;
        p.bounces++;
      }
...
function buildCpuCandidates(state, unit) {
  ...
  const budget = state.turn.movementLeft, candidates = [], keySet = /* @__PURE__ */ new Set(), bestPathCost = /* @__PURE__ */ new Map();
  const budgetFlex = 52;
  const maxDepth = 4;
  const withCannon = (candidate) => {
    const cannon = getNearbyTeamCannon(state, unit.team, candidate.x, candidate.y, 30);
    if (cannon)
      candidate.cannonId = cannon.id;
    return candidate;
  };
  const push = (c) => {
  ...
  if (currentPlatform < 0) {
    push({ x: unit.x, y: unit.y, platform: -1, cost: 0, route: [] });
    return candidates;
  }
  ...
  const explore = (platformIndex, currentX, cost, route, depth) => {
    ...
    for (const point of points) {
      ...
      if (totalCost > budget + budgetFlex)
        continue;
...
function scoreCandidateShot(state, candidate, angle, charge, actingTeam, priorityEnemyIds, primaryTarget, shotConfig) {
  ...
  for (const unit of state.units) {
    ...
    if (unit.team === actingTeam) {
      ...
      ed += damage;
      if (priorityEnemyIds.has(unit.id))
        focusDamage += damage;
      if (damage >= unit.health)
        ek++;
    }
...
function chooseAiPlan(state, unit) {
  ...
  for (const candidate of candidates) {
    ...
    const sameAsLastMove = Boolean(
      ai.lastDecision && Math.abs(candidate.x - ai.lastDecision.moveX) <= 14 && Math.abs(candidate.y - ai.lastDecision.moveY) <= 10
    );
    ...
    const variants = [{
      ...
      angleMin: AIM_MIN,
      angleMax: AIM_MAX,
      localAngleStep: angleStep,
      chargeStart: 0.16,
      localChargeStep: chargeStep,
    ...
    if (canUseCannon) {
      variants.push({
        ...
        chargeStart: 0.1,
        localChargeStep: Math.max(0.06, chargeStep - 0.01),
        shotConfig: {
          originX: cannon.x,
          originY: cannon.y,
          speedBonus: cannon.powerBoost ?? CANNON_SPEED_BONUS,
          windFactor: CANNON_WIND_FACTOR,
          spawnOffsetX: 22,
          spawnOffsetY: -18
        },
    ...
    for (const variant of variants) {
      for (let angle = variant.angleMin; angle <= variant.angleMax; angle += variant.localAngleStep) {
        for (let charge = variant.chargeStart; charge <= 1.001; charge += variant.localChargeStep) {
          ...
          if (shot.enemyDamage <= 0 && shot.focusDamage <= 0 && shot.minEnemyDist > BLAST_RADIUS * 1.2 && ai.confidence > 0.45)
            continue;
          const exploration = (state.rng() - 0.5) * (1 - ai.confidence) * 8;
          const aggressionBonus = ai.aggression * 10;
          const total = shot.score + positionScore + aggressionBonus + advanceScore + variant.cannonBonus + mobilityIntent - campPenalty - stagnationPenalty - variant.cannonPenalty + exploration;
          if (!best || total > best.totalScore) {
            best = {
              ...
              useCannon: variant.useCannon,
              cannonId: variant.cannonId
            };
          }
        }
      }
  ...
  if (!best) {
    ...
    return {
      ...
      route: [],
      positionScore: 0,
      useCannon: canFallbackCannon,
      cannonId: canFallbackCannon ? fallbackCannon.id : null
    };
  }
  return best;
}
function tuneAiPlanShot(state, plan) {
  if (!plan)
    return { angle: defaultAim("blue"), charge: 0.58 };
  const ai = state.ai;
...
function registerAiDetonationFeedback(state, projectile, outcome) {
  if (!projectile?.ownerTeam || !TEAM[projectile.ownerTeam].cpu)
    return;
  const ai = state.ai;
  ...
  if (target && target.alive) {
    ...
    if (Math.abs(dxError) > 38)
      ai.chargeBias = clamp(ai.chargeBias + (dxError * throwDir > 0 ? 0.012 : -0.012), -0.2, 0.2);
    else
      ai.chargeBias *= 0.92;
    const dyError = target.y - 24 - outcome.landingY;
    if (Math.abs(dyError) > 34)
      ai.aimBias = clamp(ai.aimBias + (dyError > 0 ? -0.42 : 0.42), -9, 9);
    else
      ai.aimBias *= 0.9;
  } else {
    ai.aimBias *= 0.95;
  ...
  ai.recentShots.push({
    ...
    targetId: projectile.intendedTargetId ?? null,
    enemyDamage: Math.round(outcome.enemyDamage),
    allyDamage: Math.round(outcome.allyDamage),
    enemyKills: outcome.enemyKills,
    allyKills: outcome.allyKills,
    landingX: Math.round(outcome.landingX),
    landingY: Math.round(outcome.landingY)
  });
  if (ai.recentShots.length > AI_RECENT_HISTORY)
    ai.recentShots.shift();
}
function moveCpuTowardPlan(state, unit, dt) {
  ...
  if (route.length > 0) {
    if (!Number.isInteger(plan.routeIndex))
      plan.routeIndex = 0;
    while (plan.routeIndex < route.length && route[plan.routeIndex].done)
      plan.routeIndex++;
  ...
  if (routeDone && Math.abs(unit.x - plan.moveX) > 8)
...
function spawnProjectile(state, unit) {
  ...
  state.projectile = {
    x: originX + Math.cos(rad) * spawnOffsetX,
    y: originY + spawnOffsetY + Math.sin(rad) * 8,
    vx: Math.cos(rad) * speed,
    vy: Math.sin(rad) * speed,
    radius: 6,
    fuseMs: 1800,
    ...
    intendedAimDeg: state.turn.aimDeg,
    intendedCharge: state.turn.charge,
    windFactor,
    source: canUseCannon ? "cannon" : "hand"
  };
  if (!canUseCannon) {
    state.turn.usingCannon = false;
    state.turn.cannonId = null;
  }
...
function detonate(state, x, y) {
  ...
  for (const unit of state.units) {
    ...
    unit.vx += nx * impulse;
    unit.vy -= Math.abs(ny) * impulse + 130 * ratio;
    unit.onGround = false;
    if (unit.health <= 0)
      unit.alive = false;
  }
...
function applyProjectilePhysics(state, dt) {
  ...
  const px = p.x, py = p.y;
  p.vx += state.wind * dt * (p.windFactor ?? 0.4);
  p.vy += GRAVITY * dt;
  p.x += p.vx * dt;
  p.y += p.vy * dt;
  p.fuseMs -= dt * 1e3;
...
function handleHumanTurn(state, input, dt) {
  ...
  if (state.turn.usingCannon) {
    const activeCannon2 = getCannonById(state, state.turn.cannonId);
    if (!activeCannon2 || !nearbyCannon || nearbyCannon.id !== activeCannon2.id) {
      state.turn.usingCannon = false;
      state.turn.cannonId = null;
    }
  }
  if (toggleCannon && !state.turn.acted && !state.projectile) {
    if (state.turn.usingCannon) {
      ...
      state.turn.usingCannon = true;
      state.turn.cannonId = nearbyCannon.id;
    }
  }
  ...
  if (ptr.inside) {
    ...
    if (Math.hypot(dx, dy) > 14)
      state.turn.aimDeg = normalizeAngleDeg(Math.atan2(dy, dx) * 180 / Math.PI);
...
function adoptCpuPlan(state, unit, plan) {
  ...
  state.ai.plan = plan;
  state.ai.targetId = plan.targetId ?? null;
  state.turn.usingCannon = Boolean(plan.useCannon);
  state.turn.cannonId = plan.useCannon ? plan.cannonId ?? null : null;
  state.ai.desiredAim = tuned.angle;
  state.ai.desiredCharge = tuned.charge;
  state.ai.lastDecision = {
    turn: state.turn.number,
    targetId: state.ai.targetId,
    platform: plan.platform,
    moveX: Math.round(plan.moveX),
    moveY: Math.round(plan.moveY),
    angle: Math.round(tuned.angle * 10) / 10,
    charge: Math.round(tuned.charge * 1e3) / 1e3,
    totalScore: Math.round((plan.totalScore ?? 0) * 10) / 10,
    useCannon: Boolean(plan.useCannon),
    routeLen: plan.route?.length ?? 0
  };
}
function handleCpuTurn(state, dt) {
  ...
  const aimFace = () => {
    const r = state.turn.aimDeg * Math.PI / 180, c = Math.cos(r);
    if (c > 0.06)
      unit.facing = 1;
    else if (c < -0.06)
      unit.facing = -1;
  };
  if (state.ai.lastMoveX == null)
    state.ai.lastMoveX = unit.x;
  if (state.ai.stage === "planning") {
    state.turn.state = "planning";
    state.turn.charging = false;
    state.turn.charge = 0;
    state.ai.timerMs -= dt * 1e3;
  ...
  if (state.ai.stage === "moving") {
    ...
    if (moved < 0.35 && Math.abs(unit.vx) < 4 && unit.onGround)
      state.ai.stuckMs += dt * 1e3;
    else
      state.ai.stuckMs = Math.max(0, state.ai.stuckMs - dt * 380);
    state.ai.lastMoveX = unit.x;
    state.ai.repathMs += dt * 1e3;
  ...
  if (state.ai.stage === "aiming") {
    ...
    aimFace();
    state.ai.timerMs = Math.max(0, state.ai.timerMs - dt * 1e3);
  ...
  if (state.ai.stage === "charging") {
    ...
    if (state.turn.charge >= dc - 6e-3 && Math.abs(state.turn.aimDeg - da) < 2.2) {
      spawnProjectile(state, unit);
      state.ai.stage = "idle";
      state.ai.plan = null;
      state.ai.timerMs = 0;
...
function maybeAdvanceTurn(state, dt) {
  ...
  if (state.turn.acted) {
    if (state.turn.settleMs > 0) {
      state.turn.settleMs = Math.max(0, state.turn.settleMs - dt * 1e3);
      state.turn.state = "resolving";
      return;
    }
    state.turn.number++;
...
function stepState(state, input, dt) {
  ...
  state.turn.remainingMs = Math.max(0, state.turn.remainingMs - dt * 1e3);
...
function drawCloud(ctx, c) {
  ...
  for (const [ox, oy, rx, ry] of [
    ...
    [0, -c.h * 0.1, c.w * 0.32, c.h * 0.55],
    [c.w * 0.26, c.h * 0.04, c.w * 0.25, c.h * 0.44],
    [-c.w * 0.12, c.h * 0.2, c.w * 0.22, c.h * 0.38],
    [c.w * 0.12, c.h * 0.18, c.w * 0.2, c.h * 0.36]
  ]) {
    ctx.beginPath();
...
function drawPlatform(ctx, p, map) {
  if (p.type === "log") {
    ...
    ctx.beginPath();
    ctx.roundRect(p.x + 3, p.y + 5, p.w, p.h + 4, 3);
    ctx.fill();
    ...
    for (let x = p.x + 4; x < p.x + p.w - 8; x += segW) {
      ...
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(x + segW - 1, p.y + 2);
      ctx.lineTo(x + segW - 1, p.y + p.h - 2);
      ctx.stroke();
      const kx = x + segW / 2, ky = p.y + p.h / 2;
      ctx.fillStyle = "#8B5014";
      ctx.beginPath();
      ctx.ellipse(kx, ky, 6, 5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = "rgba(60,30,5,0.7)";
      ctx.lineWidth = 1;
      ctx.stroke();
      ctx.fillStyle = "rgba(200,140,60,0.35)";
      ctx.beginPath();
      ctx.ellipse(kx - 1.5, ky - 1.5, 3, 2.5, 0, 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  if (p.type === "stone") {
    ...
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.moveTo(p.x + p.w * 0.28, p.y + 6);
    ctx.lineTo(p.x + p.w * 0.22, p.y + p.h * 0.45);
    ctx.lineTo(p.x + p.w * 0.3, p.y + p.h * 0.7);
    ctx.moveTo(p.x + p.w * 0.68, p.y + p.h * 0.3);
    ctx.lineTo(p.x + p.w * 0.74, p.y + p.h * 0.75);
    ctx.stroke();
  ...
  ctx.strokeStyle = adjustColor(map.groundColor, 60);
  ctx.lineWidth = 1.5;
  for (let x = p.x + 8; x < p.x + p.w - 6; x += 12) {
    const h = 4 + Math.sin(x * 0.3) * 2;
    ctx.beginPath();
    ctx.moveTo(x, p.y + 3);
    ctx.lineTo(x - 3, p.y - h);
    ctx.moveTo(x + 5, p.y + 2);
    ctx.lineTo(x + 7, p.y - h + 1);
    ctx.stroke();
  }
...
function drawStickman(ctx, unit, isActive) {
  ...
  if (alive) {
    ...
    ctx.beginPath();
    ctx.arc(x + facing * 3.5, headY - 1, 2.5, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = isFlash ? "#fff" : "#111";
    ctx.beginPath();
    ctx.arc(x + facing * 3.5, headY - 1, 1.2, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = isFlash ? "#000" : "#cc0000";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(x + facing * 2, headY + 4);
    ctx.lineTo(x + facing * 7, headY + 4);
    ctx.stroke();
  }
  ...
  ctx.beginPath();
  ctx.roundRect(x - nameW / 2, y - 62, nameW, 15, 3);
  ctx.fill();
  ctx.fillStyle = "#ffffff";
  ctx.fillText(unit.name, x, y - 55);
  if (alive) {
    const barW = 44;
    ctx.fillStyle = "rgba(0,0,0,0.6)";
    ctx.fillRect(x - barW / 2, y + 8, barW, 7);
    const hpColor = health > 60 ? "#22c55e" : health > 30 ? "#f59e0b" : "#ef4444";
    ctx.fillStyle = hpColor;
    ctx.fillRect(x - barW / 2, y + 8, barW * (health / 100), 7);
    ctx.strokeStyle = "rgba(255,255,255,0.4)";
    ctx.lineWidth = 0.8;
    ctx.strokeRect(x - barW / 2, y + 8, barW, 7);
    ctx.font = "bold 8px Arial";
    ctx.fillStyle = "#fff";
    ctx.fillText(`${health}`, x, y + 12);
  }
  if (isActive && alive) {
...
function drawCannons(ctx, state, activeUnit) {
  for (const cannon of state.cannons) {
    ...
    const isActive = Boolean(
      activeUnit && activeUnit.team === cannon.team && Math.hypot(activeUnit.x - cannon.x, activeUnit.y - cannon.y) <= CANNON_USE_RANGE && state.turn.cannonId === cannon.id && state.turn.usingCannon
    );
    ctx.save();
    ctx.translate(cannon.x, cannon.y);
    ctx.fillStyle = "rgba(0,0,0,0.28)";
    ctx.beginPath();
    ctx.ellipse(0, 7, 19, 5, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.roundRect(-16, -16, 30, 14, 5);
    ctx.fill();
    ctx.fillStyle = "#1f2937";
    ctx.beginPath();
    ctx.roundRect(8, -22, 18, 10, 3);
    ctx.fill();
    ctx.strokeStyle = isActive ? adjustColor(teamColor, 75) : "#c7d2fe";
    ctx.lineWidth = 2;
    ctx.strokeRect(-7, -22, 10, 6);
...
function drawAimLine(ctx, state, unit) {
  ...
  ctx.save();
  ctx.setLineDash([5, 4]);
  ctx.strokeStyle = useCannon ? "rgba(255,220,120,0.8)" : "rgba(255,255,255,0.65)";
  ctx.lineWidth = 1.8;
  ctx.beginPath();
  ctx.moveTo(ox, oy);
  ctx.lineTo(tx, ty);
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.arc(ox + (tx - ox) * r, oy + (ty - oy) * r, 3 - r * 0.5, 0, Math.PI * 2);
    ctx.fill();
  }
...
function drawProjectile(ctx, p) {
  ...
  ctx.beginPath();
  ctx.arc(p.x - 1, p.y - 1, p.radius * 0.55, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(180,255,100,0.3)";
  ctx.beginPath();
  ctx.arc(p.x - 2, p.y - 2, p.radius * 0.35, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "#999";
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ...
  ctx.strokeStyle = "#888";
  ctx.beginPath();
  ctx.moveTo(p.x - 3, p.y - p.radius - 4);
  ctx.lineTo(p.x + 3, p.y - p.radius - 4);
  ctx.stroke();
...
function drawExplosions(ctx, explosions, particles) {
  ...
  for (const e of explosions) {
    ...
    ctx.beginPath();
    ctx.arc(e.x, e.y, e.radius * 0.42, 0, Math.PI * 2);
    ctx.fill();
...
function drawWater(ctx, map) {
  ...
  ctx.fillRect(0, waterY, WIDTH, 7);
  const t = Date.now() / 700;
  for (let i = 0; i < 5; i++) {
    const wy = waterY + 8 + i * 13, al = 0.28 - i * 0.04;
    ctx.strokeStyle = `rgba(140,210,255,${al})`;
    ctx.lineWidth = 1.2;
    ctx.beginPath();
...
function drawBackground(ctx, state) {
  ...
  ctx.fill();
  ctx.globalAlpha = 0.2;
  ctx.fillStyle = adjustColor(map.mountainColor, -22);
  ctx.beginPath();
  shape1.map(([px, py], i) => [px, i === 0 || i === shape1.length - 1 ? py : py + 40]).forEach(([px, py], i) => i === 0 ? ctx.moveTo(px, py) : ctx.lineTo(px, py));
  ctx.closePath();
  ctx.fill();
  ctx.globalAlpha = 1;
}
function drawMapDecorations(ctx, map) {
  if (map.id === "mesa") {
    const sg = ctx.createLinearGradient(0, 445, 0, HEIGHT);
    sg.addColorStop(0, "#c2862a");
    sg.addColorStop(1, "#7a4810");
    ctx.fillStyle = sg;
    ctx.fillRect(0, 445, WIDTH, HEIGHT - 445);
    ["rgba(160,100,40,0.20)", "rgba(180,120,50,0.14)", "rgba(140,80,30,0.24)"].forEach((c, i) => {
      ctx.fillStyle = c;
      ctx.fillRect(260, 300 + i * 42, 440, 38);
    });
    const drawCactus = (cx, cy) => {
      ctx.fillStyle = "#4d7c0f";
      ctx.fillRect(cx - 5, cy, 10, 55);
      ctx.fillRect(cx - 18, cy + 14, 13, 8);
      ctx.fillRect(cx - 18, cy + 7, 8, 15);
      ctx.fillRect(cx + 5, cy + 18, 13, 8);
      ctx.fillRect(cx + 10, cy + 11, 8, 15);
    };
    drawCactus(85, 375);
    drawCactus(845, 370);
  }
  if (map.id === "graveyard") {
    ...
    ctx.fillRect(0, 462, WIDTH, HEIGHT - 462);
    ctx.fillStyle = "rgba(215,215,248,0.88)";
    ctx.beginPath();
    ctx.arc(820, 72, 32, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "rgba(110,115,145,0.60)";
    ctx.beginPath();
    ctx.arc(836, 62, 28, 0, Math.PI * 2);
    ctx.fill();
    [[60, 415], [108, 410], [188, 415], [740, 415], [802, 410], [862, 415]].forEach(([tx, ty]) => {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(tx + 9, ty - 24);
      ctx.lineTo(tx + 9, ty - 8);
      ctx.moveTo(tx + 5, ty - 18);
      ctx.lineTo(tx + 13, ty - 18);
      ctx.stroke();
    });
  ...
  if (map.id === "canyon") {
    const fg = ctx.createLinearGradient(0, 462, 0, HEIGHT);
    fg.addColorStop(0, "#8B6010");
    fg.addColorStop(1, "#4a3008");
    ctx.fillStyle = fg;
    ctx.fillRect(0, 462, WIDTH, HEIGHT - 462);
    const wc = ["rgba(120,80,20,0.3)", "rgba(100,60,10,0.2)", "rgba(150,100,30,0.25)"];
    for (let i = 0; i < 5; i++) {
      ctx.fillStyle = wc[i % 3];
      ctx.fillRect(0, 262 + i * 32, 260, 20);
      ctx.fillRect(722, 262 + i * 32, 238, 20);
    }
    [[305, 472], [425, 467], [545, 474], [665, 470]].forEach(([rx, ry]) => {
      ctx.fillStyle = "rgba(100,70,20,0.42)";
      ctx.beginPath();
      ctx.ellipse(rx, ry, 24, 11, 0, 0, Math.PI * 2);
      ctx.fill();
    });
  }
}
function drawHUD(ctx, state) {
  ...
  const secs = Math.ceil(state.turn.remainingMs / 1e3);
...
function drawTeamIcons(ctx, state) {
  ...
  const drawMiniMan = (ix, iy, alive, color) => {
    ...
    ctx.beginPath();
    ctx.arc(ix, iy - 8, 4, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = alive ? color : "rgba(80,80,80,0.4)";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(ix, iy - 4);
    ctx.lineTo(ix, iy + 4);
  ...
  for (let i = 0; i < state.teamSize; i++) {
    drawMiniMan(14 + i * 20, 52, i < reds.length, TEAM.red.color);
  }
  for (let i = 0; i < state.teamSize; i++) {
    drawMiniMan(WIDTH - 14 - i * 20, 52, i < blues.length, TEAM.blue.color);
  }
  ...
  ctx.fillRect(10, 65, 168, 9);
  ctx.fillStyle = TEAM.red.color;
  ctx.fillRect(10, 65, 168 * (rHP / maxHP), 9);
  ctx.strokeStyle = "rgba(255,255,255,0.22)";
  ctx.lineWidth = 1;
  ctx.strokeRect(10, 65, 168, 9);
  ctx.fillStyle = "rgba(0,0,0,0.55)";
  ctx.fillRect(WIDTH - 178, 65, 168, 9);
  ctx.fillStyle = TEAM.blue.color;
  ctx.fillRect(WIDTH - 178 + 168 * (1 - bHP / maxHP), 65, 168 * (bHP / maxHP), 9);
  ctx.strokeStyle = "rgba(255,255,255,0.22)";
  ctx.lineWidth = 1;
  ctx.strokeRect(WIDTH - 178, 65, 168, 9);
}
...
function snapshotOf(state) {
  ...
  return {
    ...
    ai: {
      ...
      aggression: state.ai.aggression,
      aimBias: state.ai.aimBias,
      chargeBias: state.ai.chargeBias,
      stagnantTurns: state.ai.stagnantTurns,
      lastDecision: state.ai.lastDecision,
...
function TerritoryWar() {
  ...
  const setMap = useCallback((id) => {
    ...
    s.mapId = map.id;
    s.map = map;
    s.cannons = buildMapCannons(map);
    s.turn.usingCannon = false;
    s.turn.cannonId = null;
  ...
  const tapProps = (name) => ({
    onMouseDown: () => {
      setVirtual(name, true);
      setTimeout(() => setVirtual(name, false), 80);
    },
    onTouchStart: (e) => {
      e.preventDefault();
      setVirtual(name, true);
  ...
  useEffect(() => {
    const keys = /* @__PURE__ */ new Set(["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Space", "KeyA", "KeyD", "KeyW", "KeyQ", "KeyE", "KeyX", "KeyC", "KeyR", "KeyP"]);
    const onDown = (e) => {
      if (e.code === "Enter" && stateRef.current.mode === "menu") {
        e.preventDefault();
        start();
        return;
      }
      if (!keys.has(e.code))
        return;
      e.preventDefault();
      inputRef.current.press(e.code);
    };
    const onUp = (e) => {
      if (keys.has(e.code)) {
        e.preventDefault();
        inputRef.current.release(e.code);
      }
    };
  ...
  const timerSecs = Math.ceil(sn.turn.remainingMs / 1e3);
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("div", { className: "tw-root" }, /* @__PURE__ */ React.createElement("div", { className: "tw-top-bar" }, /* @__PURE__ */ React.createElement("div", { className: "tw-title-block" }, /* @__PURE__ */ React.createElement("div", { className: "tw-title" }, "Territory War"), /* @__PURE__ */ React.createElement("div", { className: "tw-subtitle" }, "Tactical stick duel with revamped command HUD")), /* @__PURE__ */ React.createElement("div", { className: "tw-toolbar-right" }, sn.mode === "menu" && /* @__PURE__ */ React.createElement("div", { className: "tw-config" }, /* @__PURE__ */ React.createElement("label", null, /* @__PURE__ */ React.createElement("span", null, "Battlefield"), /* @__PURE__ */ React.createElement("select", { value: sn.mapId, onChange: (e) => setMap(e.target.value) }, MAPS.map((m) => /* @__PURE__ */ React.createElement("option", { key: m.id, value: m.id }, m.name, m.water ? " \u{1F30A}" : "")))), /* @__PURE__ */ React.createElement("label", null, /* @__PURE__ */ React.createElement("span", null, "Squad size"), /* @__PURE__ */ React.createElement("select", { value: sn.teamSize, onChange: (e) => setTeamSize(e.target.value) }, [1, 2, 3, 4, 5, 6].map((n) => /* @__PURE__ */ React.createElement("option", { key: n, value: n }, n, "v", n))))), /* @__PURE__ */ React.createElement("div", { className: "tw-actions" }, /* @__PURE__ */ React.createElement("button", { className: "tw-btn red-btn", onClick: start }, "\u25B6 START"), /* @__PURE__ */ React.createElement("button", { className: "tw-btn", onClick: restart }, "\u21BA RESTART"), sn.mode !== "menu" && /* @__PURE__ */ React.createElement("button", { className: "tw-btn", onClick: pause }, sn.phase === "paused" ? "\u25B6 RESUME" : "\u23F8 PAUSE")))), /* @__PURE__ */ React.createElement("div", { className: "tw-stage-wrap" }, /* @__PURE__ */ React.createElement(
    "canvas",
    {
      ref: canvasRef,
      className: "tw-canvas",
      ...
      onPointerDown: () => {
        stateRef.current.pointer.down = true;
      },
      onPointerUp: () => {
        stateRef.current.pointer.down = false;
      },
      onPointerLeave: () => {
        stateRef.current.pointer.down = false;
  ...
  ), sn.mode === "menu" && /* @__PURE__ */ React.createElement("div", { className: "tw-overlay" }, /* @__PURE__ */ React.createElement("div", { className: "tw-overlay-card" }, /* @__PURE__ */ React.createElement("h2", null, "Territory War"), /* @__PURE__ */ React.createElement("p", { style: { color: "#6688aa", marginBottom: 6, fontSize: 15 } }, MAP_DESC[sn.mapId] ?? "Tactical stickman combat."), /* @__PURE__ */ React.createElement("p", null, "Eliminate all enemies using grenades and long-range cannons. Aim carefully, manage power, and exploit terrain resources."), /* @__PURE__ */ React.createElement("br", null), /* @__PURE__ */ React.createElement("button", { className: "tw-btn red-btn", onClick: start }, "\u25B6 Start Battle"))), sn.phase === "paused" && /* @__PURE__ */ React.createElement("div", { className: "tw-overlay" }, /* @__PURE__ */ React.createElement("div", { className: "tw-overlay-card" }, /* @__PURE__ */ React.createElement("h2", null, "Paused"), /* @__PURE__ */ React.createElement("p", null, "Game is paused."), /* @__PURE__ */ React.createElement("button", { className: "tw-btn", onClick: pause }, "\u25B6 Resume"))), sn.phase === "match-over" && /* @__PURE__ */ React.createElement("div", { className: "tw-overlay" }, /* @__PURE__ */ React.createElement("div", { className: "tw-overlay-card" }, /* @__PURE__ */ React.createElement("h2", null, "Battle Over!"), /* @__PURE__ */ React.createElement("div", { className: "tw-winner-name", style: { color: sn.winner === "red" ? "#ff4444" : sn.winner === "blue" ? "#4488ff" : "#888" } }, sn.winner === "draw" ? "Stalemate!" : sn.winner === "red" ? `${TEAM.red.label} win!` : `${TEAM.blue.label} win!`), /* @__PURE__ */ React.createElement("button", { className: "tw-btn red-btn", onClick: restart }, "\u21BA Play Again")))), sn.mode !== "menu" && /* @__PURE__ */ React.createElement("div", { className: "tw-hud" }, /* @__PURE__ */ React.createElement("div", { className: "tw-hud-portrait" }, /* @__PURE__ */ React.createElement("div", { className: "tw-portrait-label" }, TEAM[sn.turn.team].cpu ? "IA" : "Usuario"), /* @__PURE__ */ React.createElement("div", { className: "tw-portrait-face", style: { background: sn.turn.team === "red" ? "#2a0808" : "#08082a", border: `2px solid ${TEAM[sn.turn.team].color}` } }, /* @__PURE__ */ React.createElement("svg", { width: "44", height: "52", viewBox: "0 0 44 52" }, /* @__PURE__ */ React.createElement("circle", { cx: "22", cy: "11", r: "9", fill: "#222", stroke: TEAM[sn.turn.team].color, strokeWidth: "2" }), /* @__PURE__ */ React.createElement("rect", { x: "17", y: "5", width: "10", height: "7", fill: TEAM[sn.turn.team].color, rx: "1" }), /* @__PURE__ */ React.createElement("circle", { cx: "25", cy: "11", r: "2", fill: "#fff" }), /* @__PURE__ */ React.createElement("line", { x1: "22", y1: "20", x2: "22", y2: "36", stroke: "#222", strokeWidth: "2.5", strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("line", { x1: "11", y1: "27", x2: "33", y2: "27", stroke: "#222", strokeWidth: "2.5", strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("line", { x1: "22", y1: "36", x2: "14", y2: "46", stroke: "#222", strokeWidth: "2.5", strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("line", { x1: "22", y1: "36", x2: "30", y2: "46", stroke: "#222", strokeWidth: "2.5", strokeLinecap: "round" }))), /* @__PURE__ */ React.createElement("div", { className: "tw-portrait-name" }, activeUnit?.name ?? "\u2014"), /* @__PURE__ */ React.createElement("div", { className: "tw-portrait-hp", style: { color: activeUnit?.health > 60 ? "#66dd66" : activeUnit?.health > 30 ? "#ffcc44" : "#ff4444" } }, activeUnit?.alive ? `${activeUnit.health}/100` : "---")), /* @__PURE__ */ React.createElement("div", { className: "tw-hud-controls" }, /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-group tw-ctrl-group-move" }, /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-label" }, "Direction"), /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-row" }, /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("left") }, "\u25C0"), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("right") }, "\u25B6"))), /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-group tw-ctrl-group-jump" }, /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-label" }, "Jump"), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn wide", ...tapProps("jump") }, "\u25B2 Jump")), /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-group tw-ctrl-group-angle" }, /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-label" }, "Angle"), /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-row" }, /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("aimUp") }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("aimDown") }, "\u25BC")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#aaa", textAlign: "center" } }, Math.round(sn.turn.aimDeg), "\xB0")), /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-group tw-ctrl-group-throw" }, /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-label" }, "Throw (hold)"), /* @__PURE__ */ React.createElement("div", { className: "tw-ctrl-row" }, /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn wide red-act", ...holdProps("fire") }, sn.turn.charging ? "\u{1F525} Hold!" : "\u26A1 SPACE"), /* @__PURE__ */ React.createElement(
    "button",
    {
      className: "tw-arr-btn",
      onMouseDown: () => inputRef.current.press("KeyC"),
      onMouseUp: () => inputRef.current.release("KeyC"),
      onTouchStart: (e) => {
        e.preventDefault();
        inputRef.current.press("KeyC");
      },
      onTouchEnd: (e) => {
        e.preventDefault();
        inputRef.current.release("KeyC");
      }
    },
    ...
    "button",
    {
      className: "tw-arr-btn cancel-btn",
      onMouseDown: () => inputRef.current.press("KeyX"),
      onMouseUp: () => inputRef.current.release("KeyX"),
      onTouchStart: (e) => {
        e.preventDefault();
        inputRef.current.press("KeyX");
      },
      onTouchEnd: (e) => {
        e.preventDefault();
        inputRef.current.release("KeyX");
      }
    },
    "X Cancel"
  )))), /* @__PURE__ */ React.createElement("div", { className: "tw-hud-power" }, /* @__PURE__ */ React.createElement("div", { className: "tw-power-label" }, "Power"), /* @__PURE__ */ React.createElement("div", { className: "tw-power-bar-outer" }, /* @__PURE__ */ React.createElement("div", { className: "tw-power-fill", style: {
    width: `${powerPct}%`,
    backgroundPosition: `${100 - powerPct}% 0`
  } })), /* @__PURE__ */ React.createElement("div", { className: "tw-power-pct" }, Math.round(powerPct), "%"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#666", marginTop: 2 } }, "Wind: ", sn.wind < 0 ? "\u25C0" : "\u25B6", Math.abs(Math.round(sn.wind)))), /* @__PURE__ */ React.createElement("div", { className: "tw-hud-timer" }, /* @__PURE__ */ React.createElement("div", { className: "tw-timer-label" }, "Time"), /* @__PURE__ */ React.createElement("div", { className: "tw-timer-value", style: { color: timerColor } }, timerSecs), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#555" } }, "Turn ", sn.turn.number)), /* @__PURE__ */ React.createElement("div", { className: "tw-hud-options" }, /* @__PURE__ */ React.createElement("div", { className: "tw-opts-label" }, "Options"), /* @__PURE__ */ React.createElement("div", { className: "tw-opts-row" }, /* @__PURE__ */ React.createElement("span", null, TEAM.red.label), /* @__PURE__ */ React.createElement("strong", { style: { color: "#cc4444" } }, sn.teams.red.alive, "/", sn.teams.red.total)), /* @__PURE__ */ React.createElement("div", { className: "tw-opts-row" }, /* @__PURE__ */ React.createElement("span", null, TEAM.blue.label), /* @__PURE__ */ React.createElement("strong", { style: { color: "#4466ee" } }, sn.teams.blue.alive, "/", sn.teams.blue.total)), /* @__PURE__ */ React.createElement("div", { className: "tw-opts-row" }, /* @__PURE__ */ React.createElement("span", null, "Map"), /* @__PURE__ */ React.createElement("strong", null, sn.mapId)), /* @__PURE__ */ React.createElement("div", { className: "tw-opts-row" }, /* @__PURE__ */ React.createElement("span", null, "Weapon"), /* @__PURE__ */ React.createElement("strong", null, sn.turn.usingCannon ? "Cannon" : "Grenade")), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, display: "flex", flexDirection: "column", gap: 3 } }, /* @__PURE__ */ React.createElement("button", { className: "tw-btn", style: { fontSize: 12, padding: "3px 8px" }, onClick: restart }, "\u21BA Restart")))), sn.mode !== "menu" && /* @__PURE__ */ React.createElement("div", { style: { width: "100%", maxWidth: 980, display: "flex", gap: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("div", { className: "tw-info-panel", style: { fontSize: 13, color: "#667788", lineHeight: 1.8 } }, /* @__PURE__ */ React.createElement("span", { className: "tw-info-title" }, "Control Guide"), /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "A/D"), " Move \xA0\xB7\xA0 ", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "W"), " Jump \xA0\xB7\xA0", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "Q/E"), " Aim \xA0\xB7\xA0 ", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "Space"), " Charge & Throw \xA0\xB7\xA0", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "C"), " Cannon \xA0\xB7\xA0 ", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "X"), " Cancel \xA0\xB7\xA0 ", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "P"), " Pause \xA0\xB7\xA0", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "R"), " Restart \xA0\xB7\xA0 ", /* @__PURE__ */ React.createElement("span", { style: { color: "#eee" } }, "Mouse"), " Aim angle"), /* @__PURE__ */ React.createElement("div", { className: "tw-info-panel" }, /* @__PURE__ */ React.createElement("div", { className: "tw-info-title" }, TEAM.red.label), sn.units.filter((u) => u.team === "red").map((u) => /* @__PURE__ */ React.createElement("div", { key: u.id, className: "tw-info-stat", style: { opacity: u.alive ? 1 : 0.3 } }, /* @__PURE__ */ React.createElement("span", null, u.name), /* @__PURE__ */ React.createElement("strong", { style: { color: u.health > 60 ? "#44cc44" : u.health > 30 ? "#ffcc00" : "#ff4444" } }, u.alive ? `${u.health}HP` : "\u2716")))), /* @__PURE__ */ React.createElement("div", { className: "tw-info-panel" }, /* @__PURE__ */ React.createElement("div", { className: "tw-info-title" }, TEAM.blue.label), sn.units.filter((u) => u.team === "blue").map((u) => /* @__PURE__ */ React.createElement("div", { key: u.id, className: "tw-info-stat", style: { opacity: u.alive ? 1 : 0.3 } }, /* @__PURE__ */ React.createElement("span", null, u.name), /* @__PURE__ */ React.createElement("strong", { style: { color: u.health > 60 ? "#44cc44" : u.health > 30 ? "#ffcc00" : "#ff4444" } }, u.alive ? `${u.health}HP` : "\u2716"))))), isTouch && sn.mode !== "menu" && /* @__PURE__ */ React.createElement("div", { className: "tw-touch-controls" }, /* @__PURE__ */ React.createElement("div", { className: "tw-touch-group" }, /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("left") }, "\u25C0"), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("right") }, "\u25B6"), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...tapProps("jump") }, "\u25B2")), /* @__PURE__ */ React.createElement("div", { className: "tw-touch-group" }, /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("aimUp") }, "Aim\u25B2"), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn", ...holdProps("aimDown") }, "Aim\u25BC"), /* @__PURE__ */ React.createElement(
    "button",
    {
      className: "tw-arr-btn",
      onMouseDown: () => inputRef.current.press("KeyC"),
    ...
    "C"
  ), /* @__PURE__ */ React.createElement("button", { className: "tw-arr-btn red-act", ...holdProps("fire") }, "\u{1F525}")))));
}
export {
  TerritoryWar as default
};
